...
const S = {
  ...
  input: { width: "100%", boxSizing: "border-box", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(34,197,94,0.3)", borderRadius: 12, padding: "13px 16px", color: "#ecfdf5", fontSize: 14, outline: "none", fontFamily: "inherit", caretColor: "#4ade80", transition: "border 0.2s, box-shadow 0.2s" },
  uploadBox: { border: "1.5px dashed rgba(34,197,94,0.2)", borderRadius: 14, padding: "24px 16px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", cursor: "pointer", marginBottom: 16, background: "rgba(255,255,255,0.02)", transition: "background 0.2s" },
  ...
  submitBtn: { width: "100%", padding: "13px", background: "linear-gradient(135deg, #14532d, #16a34a)", border: "none", borderRadius: 13, color: "#fff", fontSize: 15, fontWeight: 600, cursor: "pointer", boxShadow: "0 6px 24px rgba(22,163,74,0.3)", fontFamily: "inherit", letterSpacing: "0.02em", transition: "opacity 0.2s" },
  ...
  donCard: { background: "rgba(5,18,11,0.85)", border: "1px solid rgba(34,197,94,0.1)", borderRadius: 18, overflow: "hidden", backdropFilter: "blur(12px)", marginBottom: 0 },
  ...
  badgeAccepted: { display: "inline-flex", alignItems: "center", gap: 5, marginTop: 10, padding: "4px 12px", borderRadius: 100, background: "rgba(74,222,128,0.1)", border: "1px solid rgba(74,222,128,0.25)", color: "#4ade80", fontSize: 12, fontWeight: 600 },
  badgePending: { display: "inline-flex", alignItems: "center", gap: 5, marginTop: 10, padding: "4px 12px", borderRadius: 100, background: "rgba(234,179,8,0.1)", border: "1px solid rgba(234,179,8,0.25)", color: "#fbbf24", fontSize: 12, fontWeight: 600 },
};
...
const DonorDashboard = ({ donorName = "Donor" }) => {
  ...
  useEffect(() => {
    ...
    // ✅ Sirf status update karo - naya add mat karo (handleSubmit already karta hai)
    socket.on("donationAccepted", (updated) => {
      setDonations((prev) => prev.map((d) => d._id === updated._id ? { ...d, status: "accepted" } : d));
    });

    socket.on("donationCancelled", (updated) => {
      setDonations((prev) => prev.map((d) => d._id === updated._id ? { ...d, status: "pending" } : d));
    });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ Sirf yahan add karo, socket se nahi - double prevent
  ...
  const onInputFocus = (e) => { e.target.style.borderColor = "rgba(74,222,128,0.7)"; e.target.style.boxShadow = "0 0 0 3px rgba(22,163,74,0.15)"; e.target.style.background = "rgba(255,255,255,0.09)"; };
  const onInputBlur = (e) => { e.target.style.borderColor = "rgba(34,197,94,0.3)"; e.target.style.boxShadow = "none"; e.target.style.background = "rgba(255,255,255,0.06)"; };
      ...
      <div style={S.inner} className="donor-inner">
        <motion.div style={S.welcomeWrap} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          <div style={S.welcomeBadge}>
            <Gift size={20} />
            <h1 style={S.welcomeTitle}>Welcome back, {user?.name || donorName}! 🌟</h1>
          </div>
        ...
        <div className="donor-grid">
          <motion.div style={S.formCard} initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5 }}>
            ...
            {message && <div style={isSuccess ? S.msgSuccess : S.msgError}>{message}</div>}
            <form onSubmit={handleSubmit}>
              {fields.map((f) => (
                <div key={f.name} style={S.inputWrap}>
                  <label style={S.inputLabel}>{f.label}</label>
                  <input type={f.type} name={f.name} placeholder={f.placeholder} value={formData[f.name]} onChange={handleChange} onFocus={onInputFocus} onBlur={onInputBlur} style={S.input} />
                </div>
              ))}
              <div style={S.inputWrap}>
                <label style={S.inputLabel}>Food Photo</label>
                <label style={S.uploadBox} onMouseEnter={e => e.currentTarget.style.background = "rgba(34,197,94,0.05)"} onMouseLeave={e => e.currentTarget.style.background = "rgba(255,255,255,0.02)"}>
                  <Upload size={28} color="rgba(74,222,128,0.6)" />
                  <span style={S.uploadLabel}>Click to upload food photo</span>
                  <input type="file" accept="image/*" onChange={handleImageChange} style={{ display: "none" }} />
                  {preview && <img src={preview} alt="Preview" style={{ marginTop: 14, width: 120, height: 120, objectFit: "cover", borderRadius: 10, border: "1px solid rgba(74,222,128,0.2)" }} />}
                </label>
              </div>
  ...
  {donation.status === "accepted"
    ? `✅ Accepted by ${donation.acceptedBy?.name || "NGO"}`
    : "⏳ Pending NGO Approval"}
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        @media (max-width: 1024px) { .donor-root { padding: 28px 80px 60px 90px; } }
        @media (max-width: 768px) {
          .donor-root { padding: 20px 16px 48px 16px !important; }
          .donor-inner { max-width: 100%; }
          .donor-grid { grid-template-columns: 1fr !important; gap: 24px; }
          .donor-sub-grid { grid-template-columns: 1fr !important; }
