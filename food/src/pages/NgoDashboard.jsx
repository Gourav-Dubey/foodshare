...
const NgoDashboard = () => {
  const { user, token } = useAuth(); // ✅ AuthContext se lo
  ...
  useEffect(() => {
    ...
    socket.on("donationAccepted", (donation) => {
      // Pending se hata do sabke liye
      setPendingDonations((prev) => prev.filter((d) => d._id !== donation._id));

      // ✅ Accepted mein sirf tabhi add karo jab IS NGO ne accept kiya ho
  ...
  const handleAccept = async (donation) => {
    try {
      ...
      // ✅ Optimistic update - socket pe depend mat karo
      setPendingDonations((prev) => prev.filter((d) => d._id !== donation._id));
      setAcceptedDonations((prev) => {
        if (prev.find(d => d._id === donation._id)) return prev;
      ...
      alert("Failed to accept donation: " + (err.response?.data?.error || err.message));
    }
  };
  ...
  const renderDonationCard = (d, action, isPending) => (
    <div key={d._id} style={{ background: "rgba(5,18,11,0.85)", border: `1px solid ${isPending ? "rgba(234,179,8,0.15)" : "rgba(34,197,94,0.15)"}`, borderRadius: 16, padding: "16px 20px", marginBottom: 12, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, backdropFilter: "blur(12px)", position: "relative", overflow: "hidden" }}>
      ...
      <div className="ngo-card-inner" style={{ display: "flex", alignItems: "center", gap: 16, paddingLeft: 8, flex: 1 }}>
        {d.photo && (
          <img src={d.photo} alt={d.foodName} style={{ width: 72, height: 72, objectFit: "cover", borderRadius: 12, border: "1px solid rgba(34,197,94,0.2)", flexShrink: 0 }} />
        )}
        <div>
          <p style={{ fontWeight: 700, fontSize: "1rem", color: "#f0fdf4", margin: "0 0 5px" }}>{d.foodName}</p>
          {/* ✅ Donor info */}
          {d.createdBy && (
            <p style={{ fontSize: 13, color: "rgba(187,247,208,0.85)", margin: "0 0 4px", display: "flex", alignItems: "center", gap: 5 }}>
              👤 {d.createdBy.name} — {d.createdBy.email}
            </p>
          )}
          <p style={{ fontSize: 13, color: "rgba(187,247,208,0.6)", margin: "0 0 3px", display: "flex", alignItems: "center", gap: 5 }}>🍴 {d.quantity}</p>
          <p style={{ fontSize: 13, color: "rgba(187,247,208,0.6)", margin: "0 0 3px", display: "flex", alignItems: "center", gap: 5 }}>📍 {d.location}</p>
          {/* Accepted donations mein - kis NGO ne accept kiya */}
{!isPending && d.acceptedBy && (
  <p style={{ fontSize: 13, color: "#4ade80", margin: "0 0 4px", display: "flex", alignItems: "center", gap: 5 }}>
    🏢 Accepted by: {d.acceptedBy?.name || "NGO"}
  </p>
)}

          <p style={{ fontSize: 13, color: "rgba(187,247,208,0.45)", margin: "0 0 3px", display: "flex", alignItems: "center", gap: 5 }}>⏰ Expiry: {formatDate(d.expiry)} {formatTime(d.expiry)}</p>
          <p style={{ fontSize: 12, color: "rgba(187,247,208,0.3)", margin: 0, display: "flex", alignItems: "center", gap: 5 }}>🕐 Submitted: {formatDate(d.createdAt)} {formatTime(d.createdAt)}</p>
        </div>
      </div>
      {action}
    </div>
    ...
    <div className="ngo-root" style={{ minHeight: "100vh", background: "#080f0b", fontFamily: "'DM Sans', system-ui, sans-serif", position: "relative", overflow: "hidden" }}>
      ...
      <div className="ngo-inner" style={{ position: "relative", zIndex: 1 }}>
        <div style={{ marginBottom: 40 }}>
          ...
          <h1 style={{ fontSize: "clamp(1.8rem, 4vw, 2.6rem)", fontWeight: 800, color: "#f0fdf4", margin: 0, letterSpacing: "-0.03em" }}>NGO Dashboard</h1>
   ...
   ! Manage incoming food donations in real-time.
        ...
        <div className="ngo-grid">
          <section>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
              ...
              <span style={{ marginLeft: "auto", background: "rgba(234,179,8,0.12)", border: "1px solid rgba(234,179,8,0.25)", color: "#fbbf24", borderRadius: 100, padding: "2px 10px", fontSize: 12, fontWeight: 700 }}>{pendingDonations.length}</span>
            </div>
            {pendingDonations.length === 0 ? (
              <div style={{ background: "rgba(5,18,11,0.6)", border: "1px solid rgba(34,197,94,0.08)", borderRadius: 16, padding: "32px 20px", textAlign: "center", color: "rgba(187,247,208,0.3)", fontSize: "0.9rem" }}>No pending donations.</div>
            ) : (
              pendingDonations.map((d) => renderDonationCard(d,
                <button onClick={() => handleAccept(d)} style={{ display: "flex", alignItems: "center", gap: 6, background: "linear-gradient(135deg, #14532d, #16a34a)", color: "#fff", border: "1px solid rgba(74,222,128,0.25)", borderRadius: 10, padding: "9px 18px", fontSize: 13, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0, fontFamily: "inherit" }}
          ...
          <section>
            ...
            {acceptedDonations.length === 0 ? (
              <div style={{ background: "rgba(5,18,11,0.6)", border: "1px solid rgba(34,197,94,0.08)", borderRadius: 16, padding: "32px 20px", textAlign: "center", color: "rgba(187,247,208,0.3)", fontSize: "0.9rem" }}>No accepted donations yet.</div>
            ) : (
              acceptedDonations.map((d) => renderDonationCard(d,
                <button onClick={() => handleCancel(d)} style={{ display: "flex", alignItems: "center", gap: 6, background: "rgba(239,68,68,0.1)", color: "#fca5a5", border: "1px solid rgba(239,68,68,0.25)", borderRadius: 10, padding: "9px 18px", fontSize: 13, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0, fontFamily: "inherit" }}
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        @media (max-width: 1024px) { .ngo-root { padding: 72px 20px 60px 90px; } }
        @media (max-width: 768px) {
          .ngo-root { padding: 72px 14px 48px 14px !important; }
          .ngo-inner { max-width: 100%; }
          .ngo-grid { grid-template-columns: 1fr !important; gap: 32px; }
          .ngo-card-inner { flex-wrap: wrap; gap: 12px; }
        }
      `}</style>
    </div>
  );
